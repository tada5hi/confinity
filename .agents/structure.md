...
│   └── store/
│       ├── index.ts           # Barrel: re-exports fs + module + types
│       ├── module.ts          # Store (implements IStore) — pure in-memory query/merge engine
│       ├── fs.ts              # FSStore extends Store — filesystem load/loadFile
...
| `get<T>(key)`                 | public      | Resolves a dotted key (or array of keys) across elements, merged.          |
